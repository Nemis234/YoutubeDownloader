...
class DownloadGUI():
    ...
    def __init__(self,root:tk.Misc,package:tuple=(object,int|None,int|None,dir,str),will_concate:bool = False):
        ...
        yt,video_stream_no,audio_stream_no,directory,prefix = package
    ...
    def run(self):
        yt,video_stream_no,audio_stream_no,directory,prefixes = self.package
        ...
        if not video_stream_no == None:
            prefix = prefixes[0]
            vid_package = ((yt,video_stream_no,directory,prefix,isaudio))
            vid_work = DownloadThread(self,vid_package,DownloadGUI.numb)
            DownloadGUI.numb+=1
            vid_work.start()

        if not audio_stream_no == None and False:
            isaudio=True
            prefix = prefixes[1]
            aud_package = ((yt,audio_stream_no,directory,prefix,isaudio))
...
class DownloadThread(threading.Thread):
    ...
    """
    def __init__(self,ui:tk.Misc,package:tuple=(object,int|None,dir,str:="",bool),numb:int = 0):
    ...
    def run(self):
        yt,stream_numb,directory,prefix,isaudio = self.package
        print(yt)
        self.is_paused = self.is_cancelled = False
        ...
        invalid = '<>:"/\|?* '
        s = "".join([x for x in s if x not in invalid])
        print("prefix",prefix)
...
if __name__ == "__main__":
    ...
    def initDownload(yt:object,video_stream_no:int=None, audio_stream_no:int=None,will_concate:bool = False):
        ...
        file_name = yt.streams[0].default_filename
        print(file_name)
        
        i = 1
        while exists(directory+file_name):
            i += 1
        videoprefix = "" if i == 0 else "("+str(i)+") "

        i = 1
        while exists(directory+"Audio"+file_name):
            i += 1
        audioprefix = "" if i == 0 else "("+str(i)+") "

        print("init",yt," directory:",directory)
        print("audio",audioprefix,"video",videoprefix)
        prefixes = (videoprefix,audioprefix)
        DownloadGUI(None,(yt,video_stream_no,audio_stream_no,directory,prefixes),will_concate).run()
